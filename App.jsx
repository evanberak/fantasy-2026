...
export function buildNFLSchedule(seed) {
  ...
  for (let w = 1; w <= 18; w++) {
    ...
    for (const t of pool) {
      ...
      // prefer a division rival when one is free, otherwise anyone
      const rivals = DIVISIONS[DIV_OF[t]].filter((x) => x !== t && !used.has(x) && BYE[x] !== w);
      const others = pool.filter((x) => x !== t && !used.has(x));
      const pick = (rivals.length && rng() < 0.28) ? rivals[Math.floor(rng() * rivals.length)]
...
export function oppFor(state, team, week) {
  if (!state?.nfl) return null;
  const m = state.nfl[week - 1];
